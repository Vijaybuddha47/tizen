var listener = {
	...
	onstreamcompleted: function () {
		try {
			...
			closeVideo();
			$(".main-container").css("display", "block").show();
...
var closeVideo = function () {
	...
	if (PAGE_INDEX == 11) {
		...
		$("..program-episodes-container").addClass("active").show();
		SN.focus("#program_category_list_0");
	} else if (PAGE_INDEX == 13) {
		$(".main-container").show();
		$(".main-details-page-container").show();
		$(".video-container").removeClass("active");
		$(".main-details-page-container").addClass("active").show();
...
function show_hide_progress_bar_after_specific_time() {
	...
	if (webapis.avplay.getDuration() > 0 && (webapis.avplay.getState() == "PAUSED" || webapis.avplay.getState() == "PLAYING")) {
		if (PAGE_INDEX == 9) show_hide_video_next_previous(true);
