    ...
    $.ajax({
        ...
        success: function (data) {
            if (_.size(data) > 0) {
                ...
                $("#loader").hide();
                $("#searchResult").hide();
                $("#msg_box").html("<h2 class='search_heading'>Content not found.</h2>");
            }
        },
        error: function (xhr, error) {
            $("#loader").hide();
    ...
    $.ajax({
        ...
        success: function (data) {
            if (_.size(data) > 0) {
                ...
                str += '<span>Movies not available.</span>';
            }
        },
        error: function (xhr, error) {
            $("#loader").hide();
...
function getRedeemDetail() {
    ...
    $("#loader").show();
    // document.getElementById('search_input').value = "d3da126e7e231bcb11797f69f2dae174";
    // document.getElementById('search_input').value = "d3da126e7e231bcb11797f69f2dae174";
    $.ajax({
        type: "POST",
        ...
        async: true,
        dataType: 'json',
        data: { "user_code": localStorage.getItem('user_code'), "gift_code": $("#search_input").val() },
        // cache: false,
        crossDomain: true,
        timeout: REQUEST_TIMEOUT * 1000,
        success: function (data) {
