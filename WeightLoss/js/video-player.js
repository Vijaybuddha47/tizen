var listener = {
	...
	onstreamcompleted: function () {
		try {
			...
			// $(".main_container").show();
			// $(".home_container").css("display","block").addClass("active");
			// $(".show_detail_container").css("display","block").show();
			// SN.focus("#" + FIRST_PAGE_FOCUSED_ITEM);
			closeVideo();
			if (MENU_INDEX == 0) $(".home_container").css("display", "block").addClass("active");
			if (MENU_INDEX == 1) $(".show_container").css("display", "block").addClass("active");
			if (MENU_INDEX == 2) $(".movie_container").css("display", "block").addClass("active");
			if (MENU_INDEX == 3) $(".watchlist_container").css("display", "block").addClass("active");
			if (MENU_INDEX == 4) $(".search_container").css("display", "block").addClass("active");
			// if (MENU_INDEX == 5) $(".account_container").css("display", "block").addClass("active");
			$(".show_detail_container").css("display", "block").show();
...
var playVideo = function () {
	...
	if (document.hidden) {
		...
		if ($('#video_container').css('display', 'block')) {
			...
			// console.log("totalTrackInfo", totalTrackInfo);
			for (var i = 0; i < totalTrackInfo.length; i++) {
				// console.log(totalTrackInfo[i]['type']);
				if (totalTrackInfo[i]['type'] == 'AUDIO') {
					//console.log('audio track index is ' + totalTrackInfo[i]['index']);
					webapis.avplay.setSelectTrack('AUDIO', 1);
					//break;
				}
				else if (totalTrackInfo.type == 'TEXT') {
...
var closeVideo = function () {
	...
	$(".video_container").removeClass("active").hide();
	$(".main_container").show();
	$(".listing_container").css("display", "none").hide();
	$(".show_detail_container").addClass("active").show();
	$(".video_container").removeClass("active").hide();

	if (PAGE_INDEX == 6) {
		show_hide_screens("detail_container");
		SN.focus("movie_details_list");
	}
...
function show_hide_video_container() {
	...
	$(".home_container, .show_container, .movie_container, .watchlist_container,.seasons_container,.search_container,.detail_container,.account_container,.video-container").removeClass("active").hide();
	$(".main_container").hide();
