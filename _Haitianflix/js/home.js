...
function selected_menu_data() {
    switch (MENU_INDEX) {
        case 0:
            if (!$(".home_container").hasClass("active")) getHomeData();
            break;
        case 1:
            ...
            break;
        case 2:
            if (!$(".most_watched_container").hasClass("active")) mostWatchedData();
            break;
        case 3:
            if (!$(".favorite_container").hasClass("active")) favouriteData();
            break;
        case 4:
            if (!$(".watch_later_container").hasClass("active")) watchLaterData();
            break;
        case 5:
            if (!$(".content_partner_container").hasClass("active")) getContentPartnerData();
            break;
        case 6:
            if (!$(".redeem_ppv_container").hasClass("active")) {
                $("#search_input").val('');
                // $("#redeem_code_container").html("");
                change_screens("redeem_ppv_container");
                SN.focus("redeem_code_container");
            }
            break;
        case 7:
            if (!$(".watch_ppv_container").hasClass("active")) getWatchPPVData();
            break;
        case 8:
...
function setHomePageData(index) {
    ...
    for (var i = 0; i < totalItem; i++) {
        ...
        if (APP_DATA_ARRAY[index]['stream'][i]["type"] == "M") {
...
function rating_star() {
    ...
    if (PAGE_INDEX == 10) converted = parseFloat(SELECTED_ITEM_DATA["user_ratings"]); // Make sure we have a number
...
function episode_screen_page() {
    ...
    for (var j = 0; j < totalSeason; j++) {
        ...
        for (var i = 0; i < totalEpisode; i++) {
            ...
            // str += '<div class="thumb_icon">';
            // str += '<img class="thumbs_up" src="' + upThumb + '" alt="Thumbs up"><span class="like like_dislike_count">' + APP_EPISODE_DATA_ARRAY['show']['seasonsList'][j]['episodesList'][i]["total_like"] + '</span>';
            // str += '<img class="thumbs_down" src="' + downThumb + '" alt="Thumbs down"><span class="dislike like_dislike_count">' + APP_EPISODE_DATA_ARRAY['show']['seasonsList'][j]['episodesList'][i]["total_dislike"] + '</span>';
            // str += '</div>';
...
function set_search_result() {
    ...
    if (_.size(APP_DATA_ARRAY['search']) > 0) {
        ...
        for (var i = 0; i < totalItem; i++) {
            ...
            // str += '<div class="thumb_icon">';
            // str += '<img class="thumbs_up" src="' + upThumb + '" alt="Thumbs up">';
            // str += '<span class="like search_like">' + APP_DATA_ARRAY['search'][i]["total_like"] + '</span>';
            // str += '<img class="thumbs_down" src="' + downThumb + '" alt="Thumbs down">';
            // str += '<span class="dislike">' + APP_DATA_ARRAY['search'][i]["total_dislike"] + '</span>';
            // str += '</div>';
            str += '<div class="thumb_icon">';
...
function content_partner_screen_page() {
    ...
    if (MENU_INDEX == 5) {
        ...
        //set_content_partners_list_focus();
        SN.focus("#content_0");
        //set_content_partners_list_focus();
    }
...
function account_details() {
    ...
    if (localStorage.getItem("subscription_status") == "ACTIVE" && localStorage.getItem("days_left") != "0") {
        ...
        // $("#account_pre").text(localStorage.getItem('previous_payment'))
        // $("#account_exp").text(localStorage.getItem('next_payment'));
    } else $("#subscription_period").html('<span class="about account_exp" id="account_pre">NOT SUBSCRIBED</span>');
    if (MENU_INDEX == 8) change_screens("account_container"); SN.focus("#account_logout");
}
...
function mostWatchedPageData() {
    ...
    for (var j = 0; j < totalContent; j++) {
        ...
        for (var i = 0; i < totalItem; i++) {
            ...
            // str += '<div class="thumb_icon">';
            // str += '<img class="thumbs_up" src="' + upThumb + '" alt="Thumbs up"><span class="like like_dislike_count">' + APP_DATA_ARRAY[j]['stream'][i]["total_like"] + '</span>';
            // str += '<img class="thumbs_down" src="' + downThumb + '" alt="Thumbs down"><span class="dislike like_dislike_count">' + APP_DATA_ARRAY[j]['stream'][i]["total_dislike"] + '</span>';
            // str += '</div>';
            str += '<div class="thumb_icon">';
...
function favouritePageData() {
    ...
    if (_.size(APP_DATA_ARRAY['My_Favorite']) > 0) {
        ...
        for (var i = 0; i < totalItem; i++) {
            ...
            // str += '<div class="thumb_icon">';
            // str += '<img class="thumbs_up" src="' + upThumb + '" alt="Thumbs up"><span class="like like_dislike_count">' + APP_DATA_ARRAY['My_Favorite'][i]["total_like"] + '</span>';
            // str += '<img class="thumbs_down" src="' + downThumb + '" alt="Thumbs down"><span class="dislike like_dislike_count">' + APP_DATA_ARRAY['My_Favorite'][i]["total_dislike"] + '</span>';
            // str += '</div>';
            str += '<div class="thumb_icon">';
...
function set_watch_ppv_top_banner() {
    ...
    // str += '<li class="movie_detail movie_quality"><div class="hd">' + streamQuality + '</div></li>';
    str += '</ul>';
    ...
    // str += '<div class="Watched"><span class="title">Watched:</span><span class="detail whached">' + totalViews + '</span></div>';
    str += '<div class="Subscription"><span class="title">Subscription:</span><span class="plan_name">' + localStorage.getItem('plan_name') + '</span></div></div></div>';
    str += '<div class="banner_background"><img class="banner_back_color" src="images/banner_shadow.png" alt=""><img class="banner_back_img" id="streamPoster" onerror="poster_image_error(this);" src="' + streamPoster + '" alt="poster"></div>';
...
function search_movie_details() {
    console.log("search_movie_details.....");
    var data = {};
    var row = '';
    var index = '';
    data = APP_DATA_ARRAY, index = $("#" + FIRST_PAGE_FOCUSED_ITEM).index();
    if (SELECTED_ITEM_DATA["isLiked"] == "Y") {
        console.log("like btn...");
        $("#like_btn_img").attr("src", "images/thumbs_up.png");
        $("#dislike_btn_img").attr("src", "images/dislike.png");
    ...
    var str = '';
    streamGenre = data["search"][index]["genre"],
        streamDuration = data["search"][index]["stream_duration"],
        streamQuality = data["search"][index]["quality"],
        streamTitle = data["search"][index]["stream_title"],
        streamDescription = data["search"][index]["stream_long_description"],
        showTitle = data["search"][index]["show_name"],
        showDescription = data["search"][index]["show_short_description"],
        streamDirector = data["search"][index]["director_name"],
        streamActor = data["search"][index]["actor_name"],
        streamLanguage = data["search"][index]["language"],
        streamYear = data["search"][index]["stream_production_year"],
        showYear = data["search"][index]["show_production_year"],
        totalViews = data["search"][index]["total_views"],
        streamPoster = data["search"][index]["stream_thumbnail_2"],
        showPoster = data["search"][index]["show_thumbnail_2"];
    var totalhours = Math.floor(SELECTED_ITEM_DATA["stream_duration"] / 60) + "hr " + (SELECTED_ITEM_DATA["stream_duration"] % 60) + "Min ";
    str += '<div class="banner_detail_box">';
    str += '<ul class="movie_detail_row" id="movie_detail_row">';
    if (typeof streamGenre != "undefined") str += '<li class="movie_detail movie_category">' + streamGenre + '</li>';
    ...
    if (data["search"][index]["type"] == "M") {
        if (typeof streamTitle != "undefined") str += '<div class="movie_div"><p class="movie_name">' + streamTitle + '</p><span class="home_reddot"></span></div>';
        if (typeof streamDescription != "undefined") str += '<div class="movie_disc">' + streamDescription + '</div>';
    } else if (data["search"][index]["type"] == "S") {
        if (typeof showTitle != "undefined") str += '<div class="movie_div"><p class="movie_name">' + showTitle + '</p><span class="home_reddot"></span></div>';
        if (typeof showDescription != "undefined") str += '<div class="main_bxx"><div class="movie_disc">' + showDescription + '</div>';
    }
...
function content_partner_details_page() {
    ...
    str += '<div class="detail-contant"><span class="contant-detail">TOTAL MOVIES:&nbsp&nbsp;</span><span class="details_content">' + SELECTED_ITEM_DATA["totstream"] + '</span></div>';
    str += '<div class="detail-contant"><span class="contant-detail">TOTAL SHOW:&nbsp&nbsp;</span><span class="details_content">' + SELECTED_ITEM_DATA['totshow'] + '</span></div>';
    str += '</div>';
    ...
    // str += '<div class="text-container" id="partner_detail_button">';
    // str += '<div class="back-button focusable" tabindex="3" id="back_button" data-sn-right="null" data-sn-left="null" data-sn-up="null" data-sn-down="null">BACK</div>';
    // str += '</div>';
...
function setredeemdetailsScreen() {
    ...
    str += '<div class="info_details_heading"><span>' + APP_DATA_ARRAY["genre"] + '</span>|<span>' + Math.floor(APP_DATA_ARRAY["stream_duration"] / 60) + "hr " + (APP_DATA_ARRAY["stream_duration"] % 60) + "Min "; + '</span>';//|<span>' + APP_DATA_ARRAY["genre"] + '</span></div>';
    ...
    str += '<div class="heading_redeem">Directed By:&nbsp; ' + APP_DATA_ARRAY["director"] + ' </div>';
    str += '<div class="heading_redeem">Language:&nbsp;' + APP_DATA_ARRAY["LANGUAGE"] + '</div>';
    str += '<div class="heading_redeem">Initial Release:&nbsp;' + APP_DATA_ARRAY["stream_production_year"] + '</div>';
    str += '<div class="heading_redeem">subscription: ' + localStorage.getItem('plan_name') + '</div>';
